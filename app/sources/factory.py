...
class SourceFactory:
    ...
    _MILANUNCIOS_HOSTS = ("milanuncios.com",)
    ...
    def from_url(url: str, parse_feed_fn: Callable[[str], Optional[Any]]) -> BaseSource:
        ...
        if SourceFactory._is_milanuncios_url(url):
    ...
    @classmethod
    def _is_milanuncios_url(cls, url: str) -> bool:
        parsed = urlparse(url.strip())
        host = (parsed.netloc or "").lower()
        return any(host == domain or host.endswith(f".{domain}") for domain in cls._MILANUNCIOS_HOSTS)
